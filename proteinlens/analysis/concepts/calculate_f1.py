...
def combine_metrics_across_shards(
    ...
    for shard in tqdm(shards_to_eval, desc="Combining shard counts"):
        ...
        # check that fp, tp, and tp_per_domain are all in shard_data and have the same shape
        print(shard)
        assert "fp" in shard_data
        assert "tp" in shard_data
        assert "tp_per_domain" in shard_data
        assert (
            shard_data["fp"].shape
            == shard_data["tp"].shape
            == shard_data["tp_per_domain"].shape
        )
    ...
    metrics_df = calculate_metrics(
        ...
        fp_total,
        tp_per_domain_total,
        np.array(positive_labels),
        np.array(positive_labels_per_domain),
        concept_names,
        threshold_percents,
        is_aa_concept_list,
    )
    ...
    output_path = (
        ...
        else eval_res_dir / "concept_f1_scores.csv"
    )
