...
def load_pipeline_models(sae_dir: str, esm_model_name: str, device: str):
    """Return ``(tokenizer, esm_model, nnsight_model, sae)``."""
    ...
    print(f"  Creating NNsight wrapper …")
...
def extract_hidden_states(
    esm_model, nnsight_model, tokenizer, sequence: str, layer_idx: int, device: str,
    ...
    logits, hidden = get_esm_output_with_intervention(
        esm_model, nnsight_model, token_ids, attn_mask, layer_idx,
    )
    return logits, hidden, token_ids, attn_mask
...
def inject_and_get_logits(
    esm_model, nnsight_model, token_ids, attn_mask,
    layer_idx: int, modified_hidden: torch.Tensor,
) -> torch.Tensor:
    """Patch layer ``layer_idx`` with *modified_hidden* and return new logits."""
    logits, _ = get_esm_output_with_intervention(
        esm_model, nnsight_model, token_ids, attn_mask,
        layer_idx, modified_hidden,
    )
    return logits
...
def run_pipeline(args):
    ...
    # ── Load models ───────────────────────────────────────────────
    tokenizer, esm_model, nnsight_model, sae = load_pipeline_models(
    ...
    orig_logits, orig_hidden, token_ids, attn_mask = extract_hidden_states(
        esm_model, nnsight_model, tokenizer, sequence, args.layer, device,
    )
